...
import { assetURL, userAgentLowerCase, characterDefaultPicturePath, nonameInitialized, freezeButExtensible } from "../util/index.js";
...
class Library {
            ...
            fileReader.onload = function(fileLoadedEvent) {
              ...
              game.getDB("image", "cardback_style2", function(fileToLoad2) {
                ...
                fileReader2.onload = function(fileLoadedEvent2) {
                  ...
                  ui.css.cardback_stylesheet2 = lib.init.sheet(".card.infohidden:not(.infoflip){background-image:url(" + fileLoadedEvent2.target.result + ")}");
      ...
      config: {
        ...
        edit_character_three: {
          ...
          onclick() {
            ...
            var saveInput = function(view) {
              var code = view.state.doc.toString();
              try {
                var { character } = _exports.exec2(code);
      ...
      config: {
        ...
        edit_character: {
          ...
          onclick() {
            ...
            ui.window.classList.add("systempaused");
            var saveInput = function(view) {
              var code2 = view.state.doc.toString();
              try {
                var { character } = _exports.exec2(code2);
  ...
  codeMirrorReady(node, editor) {
    ...
    function scriptHint(editor2, keywords, getToken, options) {
      let cur = editor2.getCursor(), token = editor2.getTokenAt(cur);
      if (/\b(?:string|comment)\b/.test(token.type)) {
        return;
      }
      const innerMode = CodeMirror.innerMode(editor2.getMode(), token.state);
      ...
      while (tprop.type == "property") {
        tprop = editor2.getTokenAt(CodeMirror.Pos(cur.line, tprop.start));
        if (tprop.string != ".") {
          return;
        }
        tprop = editor2.getTokenAt(CodeMirror.Pos(cur.line, tprop.start));
      ...
      if (Array.isArray(context)) {
        try {
          const code = context.length == 1 ? context[0].string : context.reduceRight((pre, cur2) => (pre.string || pre) + "." + cur2.string);
          obj = _exports.exec(`return ${code};`, {
            event,
            trigger,
            player,
            card,
      ...
      return {
        list: [...new Set(getCompletions(token, context, keywords, options).concat(list))].filter((key) => key.startsWith(token.string)).sort((a, b) => (a + "").localeCompare(b + "")).map((text) => {
          return {
            render(elt, data, cur2) {
  ...
  genAwait(item) {
    return gnc.is.generator(item) ? gnc.of(function* () {
  ...
  linq = {
    cselector: {
      hasAttr: (name2) => `[${name2}]`,
      isAttr: (name2, item) => `[${name2}=${item}]`,
      inAttr: (name2, item) => `[${name2}~=${item}]`,
      conAttr: (name2, item) => `[${name2}*=${item}]`,
      onAttr: (name2, item) => `[${name2}|=${item}]`,
      bgnAttr: (name2, item) => `[${name2}^=${item}]`,
      endAttr: (name2, item) => `[${name2}^=${item}]`,
      merge: function() {
          ...
          switch (typeof item) {
              ...
              switch (item.constructor) {
                ...
                case null:
                  if ("_type" in item) {
                    ...
                    if (!(type in result)) {
                      result[type] = /* @__PURE__ */ Object.create(null);
                    }
                    result[type][item.name] = item.value;
                  } else {
                    if (!("style" in result)) {
                      result.style = /* @__PURE__ */ Object.create(null);
                    }
                    for (const name2 in item) {
                      result.style[name2] = item[name2];
                    }
                  }
                  break;
                default:
            ...
            case "string":
              if (/^\.|#/.test(item)) {
                ...
                for (const item2 of identities) {
                  result.identity.add(item2);
                }
              } else {
                result.content = item;
              }
    ...
    ws: {
      ...
      onmessage: function(messageevent) {
        ...
        try {
          ...
          if (game.sandbox) {
            _exports.enterSandbox(game.sandbox);
          }
          try {
    ...
    client: {
      ...
      exec: function(func) {
        ...
        if (typeof func == "function") {
          const isMarshalled = _exports.isSandboxRequired() && _exports.importSandbox().Domain.current.isFrom(func);
