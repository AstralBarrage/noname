...
class Player extends HTMLDivElement {
  ...
  constructor(position) {
    ...
    const player2 = ui.create.div(".player", position);
    Object.setPrototypeOf(player2, (lib.element.Player || Player).prototype);
    player2._args = [position];
    return player2;
  }
  build(noclick) {
    let player2 = this;
    player2.buildNode();
    player2.buildProperty();
    player2.buildExtra();
    player2.buildEventListener(noclick);
    return this;
  }
  buildNode() {
    let player2 = this;
    const node = player2.node = {
      avatar: ui.create.div(".avatar", player2, ui.click.avatar).hide(),
      avatar2: ui.create.div(".avatar2", player2, ui.click.avatar2).hide(),
      turnedover: ui.create.div(".turned", "<div>翻面<div>", player2),
      framebg: ui.create.div(".framebg", player2),
      intro: ui.create.div(".intro", player2),
      identity: ui.create.div(".identity", player2),
      hp: ui.create.div(".hp", player2),
      name: ui.create.div(".name", player2),
      name2: ui.create.div(".name.name2", player2),
      nameol: ui.create.div(".nameol", player2),
      count: ui.create.div(".count", player2).hide(),
      equips: ui.create.div(".equips", player2).hide(),
      judges: ui.create.div(".judges", player2),
      marks: ui.create.div(".marks", player2),
      chain: ui.create.div(".chain", "<div></div>", player2),
      handcards1: ui.create.div(".handcards"),
      handcards2: ui.create.div(".handcards"),
      expansions: ui.create.div(".expansions")
    };
    player2.node.handcards1._childNodesWatcher = new ChildNodesWatcher(player2.node.handcards1);
    player2.node.handcards2._childNodesWatcher = new ChildNodesWatcher(player2.node.handcards2);
  ...
  buildExtra() {
    let player2 = this;
    let node = player2.node;
    node.link = player2.mark(" ", {
      mark: get.linkintro
    });
  ...
  buildProperty() {
    let player2 = this;
    player2.phaseNumber = 0;
    player2.skipList = [];
    player2.skills = [];
    player2.invisibleSkills = [];
    player2.initedSkills = [];
    player2.additionalSkills = {};
    player2.disabledSkills = {};
    player2.hiddenSkills = [];
    player2.awakenedSkills = [];
    player2.forbiddenSkills = {};
    player2.popups = [];
    player2.damagepopups = [];
    player2.judging = [];
    player2.stat = [
      {
        card: {},
    ...
    player2.storage = {
      counttrigger: new Proxy(
        ...
        {
          get(_, prop) {
            return player2.getStat("triggerSkill")[prop];
          },
          set(_, prop, value) {
            player2.getStat("triggerSkill")[prop] = value;
            return true;
          },
          deleteProperty(_, prop) {
            delete player2.getStat("triggerSkill")[prop];
            return true;
          },
          has(_, prop) {
            return prop in player2.getStat("triggerSkill");
          },
          ownKeys() {
            return Reflect.ownKeys(player2.getStat("triggerSkill"));
          },
          getOwnPropertyDescriptor(_, prop) {
            return Object.getOwnPropertyDescriptor(player2.getStat("triggerSkill"), prop);
          }
        }
      )
    };
  ...
  buildEventListener(noclick) {
    ...
    let node = player2.node;
    if (noclick) {
      player2.noclick = true;
    } else {
      player2.addEventListener(lib.config.touchscreen ? "touchend" : "click", ui.click.target);
      node.identity.addEventListener(lib.config.touchscreen ? "touchend" : "click", ui.click.identity);
      if (lib.config.touchscreen) {
        player2.addEventListener("touchstart", ui.click.playertouchstart);
      }
    }
  }
  ...
  canRespond(event, card, type) {
    ...
    if (card) {
      return keys.some((key) => typeof key == "function" ? key(card, player2) : key == get.name(card, player2));
    }
    ...
    const cards = player2.getCards("hs", (card2) => {
      if (type === "all") {
        return true;
      }
      return evtNames.some((evtName) => {
        ...
        if (type === "respond") {
          return lib.filter.cardRespondable(card2, player2, evty);
        }
        return lib.filter.cardEnabled(card2, player2, evty);
      });
    });
    return keys.some((key) => typeof key == "function" ? cards.some((card2) => key(card2, player2)) : player2.hasUsableCard(name, type)) && !evt.directHit.includes(player2);
  }
  /**
   * 设置提示文字，有则更改，无则加之。
  ...
  addTip(index, message, isTemp = false, css = {}) {
    const player2 = this;
    if (player2.getHiddenSkills(true, true).includes(index)) {
      return;
    }
    game.broadcastAll(
      (player3, index2, message2, css2) => {
        player3.node.tipContainer ??= ui.create.div(".tipContainer", player3);
        player3.tips ??= /* @__PURE__ */ new Map();
        if (!player3.tips.has(index2)) {
          player3.tips.set(index2, ui.create.div(".tip", player3.node.tipContainer));
        }
        player3.tips.get(index2).innerHTML = message2.replace(/ /g, "&nbsp;").replace(/(?:♥︎|♦︎)/g, '<span style="color: red; ">$&</span>').replace(/\n/g, "<br>");
        player3.tips.get(index2).css(css2);
        let double = player3.classList.contains("fullskin2") && lib.config.layout !== "long2";
        const width = player3.node.avatar.clientWidth;
        let w = width * (double ? 2 : 1);
        player3.style.setProperty("--w", `${w}px`);
        game.callHook("checkTipBottom", [player3]);
      },
      player2,
      index,
      message,
    ...
    if (isTemp && !player2.storage[`temp_tip_${index}`]) {
      player2.storage[`temp_tip_${index}`] = true;
      let expire;
      if (isTemp === true) {
      ...
      player2.when(expire, false).assign({
        firstDo: true,
        priority: Infinity
      }).step(async (event, trigger, player3) => {
        delete player3.storage[`temp_tip_${index}`];
        player3.removeTip(index);
      }).finish();
    }
  }
  /**
      ...
      (player2, index2) => {
        if (index2 == void 0) {
          ...
          if (player2.tips?.has(index2)) {
            player2.tips.get(index2).remove();
            player2.tips.delete(index2);
          }
        }
        if (!player2.tips?.size) {
          player2.node.tipContainer?.remove();
          delete player2.node.tipContainer;
        }
      },
      this,
      index
  ...
  when(...triggerNames) {
    ...
    let skill = {
      ...
      get filter() {
        return (event, player3, name2) => skill.filterFuns.every((fun) => Boolean(fun(event, player3, name2))) && skill.filter2(event, player3, name2);
      },
      get filter2() {
        return (event, player3, name2) => skill.filter2Funs.length === 0 || skill.filter2Funs.some((fun) => Boolean(fun(event, player3, name2)));
      }
    };
...
`;
      ...
      const compileStep = (code, scope2) => {
        ...
        if (!scope2) {
          ...
          compiled = scope2(`(function (${params.join(", ")}) {
${body}
})`);
        }
        originals.push(compiled);
        contents.push(function(event, trigger2, player3) {
          return compiled.apply(this, [{ lib, game, ui, get, ai, _status }, event, trigger2, player3]);
        });
      };
      for (let i = 0; i < skill.contentFuns.length; i++) {
        ...
        if (typeof fun2 === "function") {
          ...
          if (_exports.isSandboxRequired()) {
            recompiledScope = scope ? _exports.eval(`return (${scope.toString()})`) : (code) => _exports.eval(`return (${code.toString()})`);
          } else {
    ...
    return {
      ...
      assign(obj) {
        ...
        if (typeof obj == "object" && obj !== null) {
          ...
          game.broadcast((skillName2, obj2) => {
   ...
   */
  recast(cards, recastingLose = (player2, cards2) => player2.loseToDiscardpile(cards2).log = false, recastingGain = (player2, cards2) => player2.draw(cards2.length).log = false) {
    ...
    if (typeof recastingLose != "function") {
      ...
      recastingLose = (player2, cards2) => player2.loseToDiscardpile(cards2).log = false;
    }
    ...
    if (typeof recastingGain != "function") {
      ...
      recastingGain = (player2, cards2) => player2.draw(cards2.length).log = false;
    }
  ...
  $syncExpand(map) {
    ...
    game.broadcast(
      function(player2, map2) {
        player2.expandedSlots = map2;
        player2.$syncExpand(map2);
      },
      this,
      map
    );
  ...
  changeZhuanhuanji(skill) {
    ...
    game.broadcastAll(
      function(player3, skill2) {
        player3.$changeZhuanhuanji(skill2);
      },
      this,
      skill
    );
  ...
  loseToSpecial(cards, tag, target) {
    ...
    next.setContent(async function(event, trigger, player2) {
      await player2.lose(event.cards, ui.special).set("getlx", false);
      let cards2 = event.cards.slice();
      cards2.removeArray(player2.getCards("hejsx"));
      if (cards2.length) {
  ...
  addGaintag(cards, tag) {
    ...
    game.broadcastAll(
      function(player2, cards2, tag2) {
        var hs = player2.getCards("hejsx");
        for (var i of cards2) {
          if (hs.includes(i)) {
            i.addGaintag(tag2);
          }
  ...
  $showCharacter(num, log) {
    ...
    game.broadcast(
      function(player2, name2, sex, num2, group) {
        player2.group = group;
        player2.name = name2;
        player2.sex = sex;
        switch (num2) {
          case 0:
            player2.classList.remove("unseen");
            player2.classList.remove("unseen_show");
            break;
          case 1:
            player2.classList.remove("unseen2");
            player2.classList.remove("unseen2_show");
            break;
          case 2:
            player2.classList.remove("unseen");
            player2.classList.remove("unseen2");
  ...
  $throwEmotion(target, name2, rotate) {
    ...
    var getLeft = function(player3) {
      if (player3 == game.me && !ui.fakeme && !ui.chess) {
        return player3.getLeft() + player3.node.avatar.offsetWidth / 2;
      }
      return player3.getLeft() + player3.offsetWidth / 2;
    };
    var player2 = this;
    ...
    var left = getLeft(player2) - width;
    var top = player2.getTop() + player2.offsetHeight / 3 - height;
  ...
  tryJudgeAnimate(bool) {
    game.broadcast(
      function(player2, bool2) {
        player2.tryJudgeAnimate(bool2);
      },
      this,
      bool
    );
  ...
  trySkillAnimate(name2, popname, checkShow) {
    ...
    var player2 = this;
    game.broadcast(
      function(player3, name3, popname2) {
        player3.trySkillAnimate(name3, popname2);
      },
      player2,
      name2,
      popname
  ...
  tryCardAnimate(card, name2, nature, popname) {
    game.broadcast(
      function(player2, card2, name3, nature2, popname2) {
        player2.tryCardAnimate(card2, name3, nature2, popname2);
      },
      this,
      card,
      name2,
  ...
  hasUsableCard(name2, type) {
    ...
    if (player2.hasCard((i) => {
      ...
      return evtNames.some((evtName) => {
        ...
        if (type === "respond") {
          return lib.filter.cardRespondable(i, player2, evt);
        }
        return lib.filter.cardEnabled(i, player2, type === "limit" ? evt : "forceEnable");
      });
    }, "hs")) {
      return true;
    }
    ...
    const skills = player2.getSkills("invisible").concat(lib.skill.global);
    ...
    for (let i = 0; i < skills.length; i++) {
      ...
      if (info.usable !== void 0) {
        ...
        if (typeof num === "function") {
          num = info.usable(skill, player2);
        }
        if (typeof num === "number" && get.skillCount(skill, player2) >= num) {
          continue;
        }
      }
      if (info.round && info.round - (game.roundNumber - player2.storage[skill + "_roundcount"]) > 0) {
        continue;
      }
      ...
      if (info.viewAs && get.is.object(info.viewAs) && info.viewAs?.name === name2) {
        const goon = !info.viewAsFilter || info.viewAsFilter(player2) !== false;
        const bool = !info.filter || typeof info.filter === "function" && evtNames.some((evtName) => {
          ...
          if (evt.name === evtName && typeof evt.filterCard == "function" && !evt.filterCard(get.autoViewAs(info.viewAs, "unsure"), player2, evt)) {
            return false;
          }
          ...
          return evt.name === evtName ? info.filter(evt, player2, evt.triggername) : true;
        });
        if (goon && bool) {
          return true;
        }
      } else if (typeof hiddenCard == "function") {
        const goon = hiddenCard(player2, name2);
  ...
  inRange(to) {
    ...
    if (game.chess) {
      ...
      for (let iwhile = 0; iwhile < totalPopulation; iwhile++) {
        if (player2.nextSeat != to) {
          player2 = player2.nextSeat;
          if (player2.isAlive() && !player2.isOut() && !player2.hasSkill("undist") && !player2.isMin(true)) {
            n++;
          }
        } else {
          break;
      ...
      if (this[i] && list) {
        ...
        if ((() => {
          ...
          if (this.skin[name2] != character) {
            ...
            game.broadcastAll(
              (player2, name3, character2, list2, origin2) => {
                player2.tempname.remove(origin2);
                player2.tempname.add(character2);
                player2.skin[name3] = character2;
                ...
                player2.smoothAvatar(name3 == "name2");
                const skinImg = lib.character[character2]?.img;
                skinImg ? player2.node["avatar" + name3.slice(4)].setBackgroundImage(skinImg) : player2.node["avatar" + name3.slice(4)].setBackground(character2, "character");
                player2.node["avatar" + name3.slice(4)].show();
                if (goon) {
    ...
    if (list && lib.characterSubstitute[this[name2]]) {
      ...
      game.broadcastAll(
        (player2, name3, character2, list2, origin2) => {
          ...
          player2.smoothAvatar(name3 == "name2");
          player2.node["avatar" + name3.slice(4)].setBackground(character2, "character");
          player2.node["avatar" + name3.slice(4)].show();
          if (goon) {
            delete lib.character[character2];
          }
    ...
    if (rawPairs.length == newPairs.length) {
      for (let i = 0; i < Math.min(2, rawPairs.length); i++) {
        ...
        if (rawName != newName && lib.character[rawName] && lib.character[newName]) {
          player2.reinit(rawName, newName, null, true);
        }
      }
    } else if (rawPairs.length == 1 && newPairs.length == 2) {
      player2.name1 = newPairs[0];
      player2.name2 = newPairs[1];
      player2.$reinit12(newPairs);
    } else if (rawPairs.length == 2 && newPairs.length == 1) {
      player2.name1 = newPairs[0];
      delete player2.name2;
      player2.$reinit21(newPairs);
    }
    if (!player2.isUnseen(1)) {
      player2.name = player2.name1;
      player2.sex = get.character(player2.name1, 0);
    } else if (!player2.isUnseen(2)) {
      player2.name = player2.name2;
      player2.sex = get.character(player2.name2, 0);
    }
  }
  $reinit12(newPairs) {
    const player2 = this;
    player2.node.avatar.setBackground(newPairs[0], "character");
    player2.node.name.innerHTML = get.slimName(newPairs[0]);
    player2.name2 = newPairs[1];
    player2.classList.add("fullskin2");
    player2.node.avatar2.classList.remove("hidden");
    player2.node.avatar2.setBackground(newPairs[1], "character");
    player2.node.name2.innerHTML = get.slimName(newPairs[1]);
    if (player2 == game.me && ui.fakeme) {
      ui.fakeme.style.backgroundImage = player2.node.avatar.style.backgroundImage;
    }
  }
  $reinit21(newPairs) {
    const player2 = this, name2 = newPairs[0];
    player2.smoothAvatar(false);
    player2.node.avatar.setBackground(name2, "character");
    player2.node.name.innerHTML = get.slimName(name2);
    player2.classList.remove("fullskin2");
    player2.node.avatar2.classList.add("hidden");
    player2.node.name2.innerHTML = "";
    if (player2 == game.me && ui.fakeme) {
      ui.fakeme.style.backgroundImage = player2.node.avatar.style.backgroundImage;
    }
  }
  reinit(from, to, maxHp, online) {
    ...
    if (!online) {
      ...
      game.broadcast(
        function(player2, from2, to2, skills) {
          player2.reinit(from2, to2, null, true);
          player2.applySkills(skills);
        },
        this,
        from,
        to,
  ...
  $uninit() {
    ...
    game.broadcastAll(function(player2) {
      delete player2.storage._disableJudge;
      for (var i = 0; i < player2.node.judges.childNodes.length; i++) {
        if (player2.node.judges.childNodes[i].name == "disable_judge") {
          player2.node.judges.removeChild(player2.node.judges.childNodes[i]);
          break;
        }
      }
    }, this);
  ...
  changeSeat(position, video) {
    var player2 = this;
    if (video !== false) {
      game.addVideo("changeSeat", player2, position);
    }
    var rect1 = player2.getBoundingClientRect();
    ...
    if ((game.chess || player2.dataset.position != 0 && position != 0) && player2.classList.contains("linked")) {
      player2.style.transform = "rotate(-90deg) translate(" + -dy + "px," + dx + "px)";
    } else {
  ...
  throwEmotion(target, emotion, rotate) {
    game.broadcastAll(
      function(player2, target2, emotion2, rotate2) {
        player2.$throwEmotion(target2, emotion2, rotate2);
      },
      this,
      target,
      emotion,
  ...
  setAvatar(name2, name22, video, fakeme) {
    ...
    game.broadcast(
      function(player2, name3, name23) {
        player2.setAvatar(name3, name23, false);
      },
      this,
      name2,
      name22
  ...
  setAvatarQueue(name2, list) {
    var node;
    var player2 = this;
    if (player2.name2 == name2) {
      node = player2.node.avatar2;
    } else {
      node = player2.node.avatar;
    }
    if (node._avatarqueue) {
      ...
      var func = function() {
        if (node._avatarqueue.length) {
          player2.setAvatar(name2, node._avatarqueue.shift(), false, false);
        } else {
          clearInterval(node._avatarqueueinterval);
          delete node._avatarqueue;
          delete node._avatarqueueinterval;
          player2.setAvatar(name2, name2, false, false);
        }
      };
  ...
  update() {
    ...
    game.broadcast(
      function(player2, hp, maxHp, hujia) {
        player2.hp = hp;
        player2.maxHp = maxHp;
        player2.hujia = hujia;
        player2.$update();
  ...
  updateMarks(connect) {
    if (typeof connect == "string" && _status.connectMode && !game.online) {
      game.broadcast(
        function(player2, storage, skill) {
          player2.storage[skill] = storage;
          player2.updateMarks();
        },
        this,
  ...
  line(target, config) {
    if (get.itemtype(target) == "players") {
      ...
      game.broadcast(
        function(player2, target2, config2) {
          player2.line(target2, config2);
        },
        this,
        target,
        config
  ...
  getHiddenSkills(unowned, unique) {
    const player2 = this;
    return player2.getStockSkills(unowned, unique, true).removeArray(player2.getStockSkills(unowned, unique));
  }
  /**
   * @param { string } [arg1='h']
  ...
  *iterableGetCards(arg1, arg2) {
   ...
   */
  *iterableGetDiscardableCards(player2, arg1, arg2) {
    for (let card of this.iterableGetCards(arg1, arg2)) {
      if (lib.filter.canBeDiscarded(card, player2, this)) {
        yield card;
      }
    }
  }
  getDiscardableCards(player2, arg1, arg2) {
    return Array.from(this.iterableGetDiscardableCards(player2, arg1, arg2));
  }
  /**
   ...
   */
  *iterableGetGainableCards(player2, arg1, arg2) {
    for (let card of this.iterableGetCards(arg1, arg2)) {
      if (lib.filter.canBeGained(card, player2, this)) {
        yield card;
      }
    }
  }
   ...
   */
  getGainableCards(player2, arg1, arg2) {
    return Array.from(this.iterableGetGainableCards(player2, arg1, arg2));
  }
  ...
  countDiscardableCards(player2, arg1, arg2) {
    return this.getDiscardableCards(player2, arg1, arg2).length;
  }
  /**
   ...
   */
  countGainableCards(player2, arg1, arg2) {
    return this.getGainableCards(player2, arg1, arg2).length;
  }
  /**
   * 返回武将牌上原有的技能
  ...
  getSkills(arg2, arg3, arg4) {
    ...
    for (let i2 in this.additionalSkills) {
      if (Array.isArray(this.additionalSkills[i2]) && (arg2 || i2.indexOf("hidden:") !== 0)) {
        for (j = 0; j < this.additionalSkills[i2].length; j++) {
          if (this.additionalSkills[i2][j]) {
            skills.add(this.additionalSkills[i2][j]);
          }
        }
      } else if (this.additionalSkills[i2] && typeof this.additionalSkills[i2] == "string") {
        skills.add(this.additionalSkills[i2]);
      }
    }
    for (let i2 in this.tempSkills) {
      skills.add(i2);
    }
    if (arg2) {
    ...
    for (let i2 in this.forbiddenSkills) {
      skills.remove(i2);
    }
    if (arg4 !== false) {
  ...
  playerfocus(time) {
    ...
    game.broadcast(
      function(player2, time2) {
        player2.playerfocus(time2);
      },
      this,
      time
    );
  ...
  insertEvent(name2, content, arg) {
    var evt = _status.event.getParent("phase");
    var next;
    if (evt && evt.parent && evt.parent.next) {
      next = game.createEvent(name2, null, evt.parent);
    } else {
      next = game.createEvent(name2);
    }
    for (var i in arg) {
      next[i] = arg[i];
    }
  ...
  chooseToDiscard() {
    ...
    next.autochoose = function() {
      ...
      for (var i2 = 0; i2 < cards.length; i2++) {
        if (!lib.filter.cardDiscardable(cards[i2], this.player, this)) {
          num--;
        }
      }
    ...
    if (Array.isArray(target)) {
      ...
      if (check) {
        ...
        next.ai = function(card) {
          ...
          var getn = function(card2) {
            if (player2.hasSkillTag("forceWin", null, { card: card2 })) {
              return 13 * (event.small ? -1 : 1);
            }
            return get.number(card2) * (event.small ? -1 : 1);
          };
          if (source && source != player2) {
            if (get.attitude(player2, source) > 1) {
              if (event.small) {
      ...
      if (check) {
        ...
        next.ai = function(card) {
          ...
          var event = _status.event.getParent();
          var to = player2 == event.player ? event.target : event.player;
          var addi = get.value(card) >= 8 && get.type(card) != "equip" ? -6 : 0;
          var friend = get.attitude(player2, to) > 0;
          if (card.name == "du") {
            addi -= 5;
          }
  ...
  showCards(cards, str, isFlash = false) {
    ...
    next.getShown = function(player2, key) {
      const event = this;
      if (get.itemtype(player2) != "player") {
        if (player2 == "others" && typeof key == "string") {
          return event.show_map?.get?.("others")?.[key] || [];
        } else if (typeof player2 == "string") {
          return event.show_map?.get?.("others")?.[player2] || [];
        }
        return null;
      }
      if (!key) {
        return event.show_map?.get?.(player2) || {};
      }
      return event.show_map?.get?.(player2)?.[key] || [];
    };
  ...
  canMoveCard(withatt, nojudge) {
    ...
    return sourceTargets.some((current) => {
      const att = get.sgn(get.attitude(player2, current));
      if (!withatt || att != 0) {
    ...
    if (Array.isArray(args)) {
      for (var i = 0; i < args.length; i++) {
        if (get.itemtype(args[i]) == "player") {
          ...
          next.bottom = true;
        } else if (typeof args[i] == "object" && args[i] && args[i].drawDeck != void 0) {
  ...
  draw() {
    ...
    for (var i = 0; i < arguments.length; i++) {
      if (get.itemtype(arguments[i]) == "player") {
        ...
        next.bottom = true;
      } else if (typeof arguments[i] == "object" && arguments[i] && arguments[i].drawDeck != void 0) {
  ...
  randomDiscard() {
    let position = "he", num = 1, delay = null;
  ...
  modedDiscard() {
    ...
    for (let skill of skills) {
      ...
      if (mod) {
        for (let i = 0; i < next.cards.length; i++) {
          let arg = [next.cards[i], next.source, this, event, "unchanged"], result = mod.call(game, ...arg);
          if (result !== void 0 && typeof arg[arg.length - 1] !== "object") {
            arg[arg.length - 1] = result;
          }
          if (!arg[arg.length - 1]) {
    ...
    next.setContent(async function(event2, trigger, player2) {
      if (event2.skills.length && event2.log) {
        for (let i of event2.skills) {
          if (typeof player2[event2.log] === "function") {
            player2[event2.log](i);
          }
        }
      }
      const cards = event2.cards;
      if (cards.length) {
        game.log(player2, "弃置了", cards);
        event2.done = player2.lose(cards, event2.position, "visible");
  ...
  $addToExpansion(cards, broadcast, gaintag, check = true) {
    ...
    if (broadcast !== false) {
      game.broadcast(
        function(player2, cards2, gaintag2, check2) {
          player2.$addToExpansion(cards2, null, gaintag2, check2);
        },
        this,
        cards,
        gaintag,
  ...
  directgain(cards, broadcast, gaintag) {
    ...
    if (broadcast !== false) {
      game.broadcast(
        function(player2, cards2) {
          player2.directgain(cards2);
        },
        this,
        cards
      );
  ...
  directgains(cards, broadcast, gaintag) {
    ...
    if (broadcast !== false) {
      game.broadcast(
        function(player2, cards2, gaintag2) {
          player2.directgains(cards2, null, gaintag2);
        },
        this,
        cards,
        gaintag
  ...
  gain() {
    ...
    next.getd = function(player2, key, position) {
      if (!position) {
    ...
    next.getl = function(player2) {
      ...
      player2.checkHistory("lose", function(evt) {
        if (evt.parent == that) {
    ...
    next.getg = function(player2) {
      if (this.getlx === false || player2 != this.player || !this.cards) {
        return [];
      }
  ...
  lose() {
    ...
    next.getd = function(player2, key, position) {
      ...
      if (this.getlx === false || this.position != position || player2 && this.player != player2 || !Array.isArray(this[key])) {
        return [];
      }
      return this[key].slice(0);
    };
    next.getl = function(player2) {
      if (this.getlx !== false && this.player == player2) {
        return this;
      }
      return {
        player: player2,
  ...
  revive(hp, log) {
    ...
    this.update();
    var player2;
    player2 = this.previousSeat;
    while (player2.isDead()) {
      player2 = player2.previousSeat;
    }
    player2.next = this;
    this.previous = player2;
    player2 = this.nextSeat;
    while (player2.isDead()) {
      player2 = player2.nextSeat;
    }
    player2.previous = this;
    this.next = player2;
  ...
  logSkill(name2, targets, nature, logv, args) {
    ...
    if (info) {
      var player2 = this;
      var players = player2.getSkills(false, false, false);
      var equips = player2.getSkills("e");
      ...
      evt.after.push(next);
      next.setContent("emptyEvent");
      player2.getHistory("useSkill").push(logInfo);
  ...
  localMarkSkill(skill, target, event) {
    const func = (skill2, player2) => {
      ...
      if (player2.marks[name2]) {
        player2.marks[name2].info = info;
      } else {
        player2.marks[name2] = player2.mark(name2, info);
      }
      player2.updateMarks();
    };
  ...
  markSkill(name2, info, card, nobroadcast) {
    ...
    const func = function(storage, player2, name3, info2, card2) {
      player2.storage[name3] = storage;
      if (!info2) {
        if (player2.marks[name3]) {
          player2.updateMarks();
          return;
        }
        if (lib.skill[name3]) {
      ...
      if (player2.marks[name3]) {
        player2.marks[name3].info = info2;
      } else {
        if (card2) {
          player2.marks[name3] = player2.mark(card2, info2, name3);
        } else {
          player2.marks[name3] = player2.mark(name3, info2);
        }
      }
      player2.updateMarks();
    };
  ...
  unmarkSkill(name2, nobroadcast) {
    ...
    if (!nobroadcast) {
      game.broadcast(
        function(player2, name3) {
          if (player2.marks[name3]) {
            player2.marks[name3].delete();
            player2.marks[name3].style.transform += " scale(0.2)";
            delete player2.marks[name3];
            ui.updatem(player2);
          }
        },
        this,
    ...
    if (this.marks[name2]) {
      ...
      if (!game.online && info && info.intro && info.intro.onunmark) {
        if (info.intro.onunmark == "throw") {
          ...
          info.intro.onunmark(this.storage[name2], this);
        } else {
          delete this.storage[name2];
        }
      }
  ...
  markSkillCharacter(id, target, name2, content, nobroadcast) {
    ...
    const func = function(player2, target2, name3, content2, id2) {
      if (player2.marks[id2]) {
        player2.marks[id2].name = name3 + "_charactermark";
        player2.marks[id2]._name = target2;
        player2.marks[id2].info = {
          name: name3,
          content: content2,
          id: id2
        };
        player2.marks[id2].setBackground(target2, "character");
        game.addVideo("changeMarkCharacter", player2, {
          id: id2,
          name: name3,
          content: content2,
          target: target2
        ...
        player2.marks[id2] = player2.markCharacter(target2, {
          name: name3,
          content: content2,
          id: id2
        });
        player2.marks[id2]._name = target2;
        game.addVideo("markCharacter", player2, {
          name: name3,
          content: content2,
          id: id2,
          target: target2
  ...
  hasUseTarget(card, distance, includecard) {
    ...
    return game.hasPlayer2(function(current) {
      return player2.canUse(card, current, distance, includecard);
    }, true);
  }
  /**
   * 场上是否存在收益为正的目标
  ...
  hasValueTarget(card, distance, includecard) {
    ...
    game.checkMod(card, player2, range, "selectTarget", player2);
    if (!range) {
      return false;
    }
    let cache = CacheContext.requireCacheContext();
    for (var i = 0; i < targets.length; i++) {
      if (player2.canUse(card, targets[i], distance, includecard)) {
        var eff = cache.get.effect(targets[i], card, player2, player2);
        if (range[1] == 1 && eff > 0) {
          return true;
        }
        value.push(eff);
  ...
  addSubPlayer(cfg) {
    ...
    game.broadcastAll(
      ...
      function(player2, skill2, list2, cfg2) {
        lib.skill[skill2] = {
          intro: {
            content: cfg2.intro || ""
          },
  ...
  removeSubPlayer(name2) {
    if (this.hasSkill("subplayer") && this.name == name2) {
      ...
      if (player.storage[name2].onremove) {
        player.storage[name2].onremove(player, name2);
      }
      this.removeSkill(name2);
      delete this.storage[name2];
      game.log(player, "牺牲了随从", "#g" + name2);
    ...
    if (Array.isArray(skill)) {
      ...
      game.broadcastAll(
        function(player2, skill2) {
          player2.invisibleSkills.remove(skill2);
        },
        this,
        skill
      );
  ...
  addAdditionalSkills(skill, skillsToAdd, keep) {
    ...
    return this.changeSkills(skillsToAdd, skillsToRemove).set("$handle", function(player2, skillsToAdd2, skillsToRemove2) {
      if (skillsToRemove2.length > 0) {
        player2.removeSkillLog(skillsToRemove2, get.event().popup);
      }
      if (skillsToAdd2.length > 0) {
        ...
        if (!Array.isArray(player2.additionalSkills[skill])) {
          player2.additionalSkills[skill] = [];
        }
        for (var i = 0; i < skillsToAdd2.length; i++) {
          player2.addSkill(skillsToAdd2[i], null, true, true);
          player2.additionalSkills[skill].push(skillsToAdd2[i]);
        }
        game.broadcast(
          (player3, map) => {
            player3.additionalSkills = map;
          },
          player2,
          player2.additionalSkills
        );
        player2.checkConflict();
      }
      _status.event.clearStepCache();
    });
  }
  ...
  getRemovableAdditionalSkills(skill, target) {
    const player2 = this, removableSkills = [];
    if (this.additionalSkills[skill]) {
      const additionalSkills = this.additionalSkills[skill];
      const hasAnotherSKill = function(skillkey, skill2) {
        return player2.skills.includes(skill2) || player2.tempSkills[skill2] || Object.keys(player2.additionalSkills).some((key) => {
          if (key === skillkey) {
            return false;
          }
          if (Array.isArray(player2.additionalSkills[key])) {
            return player2.additionalSkills[key].includes(skill2);
          }
          return player2.additionalSkills[key] == skill2;
        });
      };
  ...
  removeAdditionalSkill(skill, target) {
    const player2 = this, skills = this.getRemovableAdditionalSkills(skill, target);
    if (skills.length) {
  ...
  removeAdditionalSkills(skill, target) {
    const player2 = this, skills = this.getRemovableAdditionalSkills(skill, target);
    return player2.changeSkills([], skills).set("$handle", function(player3, addSkills, removeSkills) {
      if (removeSkills.length > 0) {
        player3.removeSkillLog(removeSkills, get.event().popup);
      }
      player3.$removeAdditionalSkills(skill, target);
    });
  }
  /**
   * 中流（×）批量重置技能
  ...
  refreshSkill(skills) {
    const player2 = this;
    if (!skills) {
      skills = game.expandSkills(player2.getStockSkills(true, true));
    }
    if (typeof skills == "string") {
      skills = [skills];
    }
    ...
    const resetSkills = [], suffixs = ["used", "round", "block", "blocker"];
    for (const skill of skills) {
      const info = get.info(skill);
      if (info.usable !== void 0) {
        if (typeof player2.getStat("triggerSkill")[skill] == "number" && player2.getStat("triggerSkill")[skill] >= 1) {
          delete player2.getStat("triggerSkill")[skill];
          resetSkills.add(skill);
        }
        if (typeof player2.getStat("skill")[skill] == "number" && player2.getStat("skill")[skill] >= 1) {
  ...
  removeVirtualJudge(VCard) {
    const player2 = this;
    game.addVideo("removeVirtualJudge", player2, get.vcardInfo(VCard));
    game.broadcast(
      (VCard2, player3) => {
        const cards2 = player3.vcardsMap?.judges;
    ...
    const cards = player2.vcardsMap?.judges;
    if (cards && cards.includes(VCard)) {
      cards.remove(VCard);
    }
    if (VCard.storage.equipEnable && VCard.cards?.some((card) => get.type(card) == "equip")) {
      const es = player2.getVCards("e"), equips = VCard.cards;
      if (equips.length) {
        let keepSkills = Object.values(player2.additionalSkills).flat().concat(get.skillsFromEquips(es)), skills = get.skillsFromEquips(equips).removeArray(keepSkills);
        if (skills.length) {
          player2.removeSkill(skills);
        }
      }
  ...
  removeVirtualEquip(VCard) {
    const player2 = this;
    game.addVideo("removeVirtualEquip", player2, get.vcardInfo(VCard));
    game.broadcast(
      (VCard2, player3) => {
        const cards2 = player3.vcardsMap?.equips;
        if (cards2 && cards2.includes(VCard2)) {
          cards2.remove(VCard2);
        }
      },
    ...
    const cards = player2.vcardsMap?.equips;
    if (cards && cards.includes(VCard)) {
      player2.removeEquipTrigger(VCard, true);
      cards.remove(VCard);
    }
    player2.$handleEquipChange();
  }
  ...
  removeSkill(skill) {
    ...
    if (Array.isArray(skill)) {
      ...
      if (skill === "counttrigger") {
        ...
        game.broadcastAll(
          function(player2, skill2) {
            player2.skills.remove(skill2);
            player2.hiddenSkills.remove(skill2);
            player2.invisibleSkills.remove(skill2);
            delete player2.tempSkills[skill2];
            for (var i2 in player2.additionalSkills) {
              player2.additionalSkills[i2].remove(skill2);
            }
          },
          this,
          skill
  ...
  addTempSkills(skillsToAdd, expire) {
    ...
    return this.changeSkills(skillsToAdd, []).set("$handle", function(player2, addSkills, removeSkills) {
      if (addSkills.length) {
        game.log(
          player2,
          "获得了技能",
        ...
        } while (player2.additionalSkills[skillName] != null);
        player2.addAdditionalSkill(skillName, skillsToAdd);
        player2.when(expire, false).assign({
          firstDo: true,
          priority: Infinity
        }).step(async (event, trigger, player3) => {
          player3.removeAdditionalSkills(skillName);
        }).finish();
      }
    });
  }
  ...
  getRoundHistory(key, filter = lib.filter.all, num, keep, last) {
    ...
    const player2 = this;
    let evts = [], history = player2.actionHistory;
  ...
  getCardUsable(card, pure) {
    ...
    if (typeof num == "function") {
      num = num(card, player2);
    }
    num = game.checkMod(card, player2, num, "cardUsable", player2);
    if (typeof num != "number") {
      return Infinity;
    }
    if (!pure && _status.currentPhase == player2) {
      return num - player2.countUsed(card);
    }
    return num;
  }
  /**
  ...
  getAttackRange(raw) {
    ...
    let range = 0;
    if (raw) {
      range = game.checkMod(player2, player2, range, "globalFrom", player2);
      range = game.checkMod(player2, player2, range, "attackFrom", player2);
      const equips = player2.getVCards("e", function(card) {
        return !card.cards?.some((card2) => {
          return ui.selected.cards?.includes(card2);
        });
      });
      ...
      return player2.getEquipRange() - range;
    }
    let base = game.checkMod(player2, "unchanged", "attackRangeBase", player2);
    if (base != "unchanged") {
      range = base;
    } else {
      range = player2.getEquipRange();
    }
    range = game.checkMod(player2, range, "attackRange", player2);
    return range;
  }
  /**
   * 返回一些牌的攻击距离
  ...
  getEquipRange(cards) {
    const player2 = this;
    if (!cards) {
      cards = player2.getVCards("e", function(card) {
        return !card.cards?.some((card2) => {
          return ui.selected.cards?.includes(card2);
        });
      });
    ...
    const range = cards.reduce((range2, card) => {
      ...
      if (info.distance) {
        if (typeof info.distance.attackRange == "function") {
          newRange = info.distance.attackRange(card, player2);
  ...
  getGlobalFrom() {
    ...
    var range = 0;
    range = game.checkMod(player2, player2, range, "globalFrom", player2);
    var equips = player2.getVCards("e", function(card) {
      return !card.cards?.some((card2) => {
        return ui.selected.cards?.includes(card2);
      });
    });
  ...
  getGlobalTo() {
    var player2 = this;
    var range = 0;
    range = game.checkMod(player2, player2, range, "globalTo", player2);
    var equips = player2.getVCards("e", function(card) {
      return !card.cards?.some((card2) => {
        return ui.selected.cards?.includes(card2);
      });
  ...
  getEnemies(func, includeDie) {
    ...
    if (mode == "identity") {
      ...
      if (player2.identity == "ye") {
        ...
        var group = lib.character[player2.name1][1];
        targets = game[method](function(target) {
          if (func && !func(target)) {
            return false;
          }
      ...
      targets = game[method](function(target) {
        ...
        return target.identity != player2.identity;
      });
    } else {
      targets = game[method](function(target) {
        if (func && !func(target)) {
        ...
        return target.side != player2.side;
      });
    }
  ...
  isFriendOf(player2) {
    if (get.mode() == "guozhan") {
      ...
      if (this.getStorage("yexinjia_friend").includes(player2) || player2.getStorage("yexinjia_friend").includes(this)) {
        return true;
      }
      if (this.identity == "unknown" || this.identity == "ye") {
        return false;
      }
      if (player2.identity == "unknown" || player2.identity == "ye") {
        return false;
      }
      return this.identity == player2.identity;
    }
    if (get.mode() == "doudizhu") {
      return this.identity == player2.identity;
    }
    if (this.side != void 0 && typeof player2.side == "boolean") {
      return this.side == player2.side;
    }
    return this == player2;
  }
  isFriendsOf(player2, includeDie) {
    return player2.getFriends(true, includeDie).includes(this);
  }
  isEnemiesOf(player2, includeDie) {
    return player2.getEnemies(null, includeDie).includes(this);
  }
  isAlive() {
  ...
  isMajor() {
    if (get.mode() == "guozhan") {
      ...
      var map = {}, sides = [], pmap = _status.connectMode ? lib.playerOL : game.playerMap, player2;
      ...
      if (!player2 || map[player2].length < 2) {
        return false;
      }
      for (var i in map) {
        if (map[i].length > map[player2].length) {
          return false;
        }
      }
      return true;
  ...
  siege(player2) {
    ...
    if (!player2) {
      ...
      return player2.sieged() && (player2.getNext() == this || player2.getPrevious() == this);
    }
  }
  sieged(player2) {
    if (this.identity == "unknown") {
  ...
  needsToDiscard(add, filter, pure) {
    ...
    if (typeof filter !== "function") {
      filter = (card, player2) => !player2.canIgnoreHandcard(card);
    }
  ...
  isZhu2() {
    var player2 = this, mode = get.mode();
    if (!this.isZhu) {
      return false;
    }
    if (mode == "identity") {
  ...
  isInitFilter(tag) {
    const player2 = this;
    for (const name2 of [player2.name, player2.name1, player2.name2]) {
  ...
  hasZhuSkill(skill, player2) {
    ...
    if (player2) {
      ...
      if (mode == "identity" && _status.mode == "purple") {
        if (this.identity.slice(0, 1) != player2.identity.slice(0, 1)) {
          return false;
        }
      }
      if (mode == "versus" && (_status.mode == "four" || _status.mode == "guandu")) {
        if (this.side != player2.side) {
          return false;
        }
      }
    }
...
hasSkillTag：`, arg);
  ...
  isUnknown(player2) {
    var mode = get.mode();
    if (mode == "identity" || mode == "guozhan") {
      if (this.ai.shown == 0 && this != player2) {
        return true;
      }
    }
    return false;
  ...
  hasCard(name2, position) {
    if (typeof name2 == "function") {
      for (let card of this.iterableGetCards(position, name2)) {
        return true;
      }
    } else {
      if (this.countCards(position, name2)) {
        return true;
      }
    }
    return false;
  ...
  $draw(num, init, config, cardsetion) {
    ...
    if (init !== false && init !== "nobroadcast") {
      game.broadcast(
        function(player2, num2, init2, config2, cardsetion2) {
          player2.$draw(num2, init2, config2, cardsetion2);
        },
        this,
        num,
        init,
  ...
  $throw(card, time, init, nosource, cardsetion) {
    ...
    if (init !== false) {
      if (init !== "nobroadcast") {
        game.broadcast(
          function(player2, card2, time2, init2, nosource2, cardsetion2) {
            player2.$throw(card2, time2, init2, nosource2, cardsetion2);
          },
          this,
          card,
          time,
  ...
  $throwordered() {
    ...
    if (lib.config.card_animation_info) {
      ...
      let eventInfo = arguments[2], player2 = this;
      if (!eventInfo) {
        eventInfo = get.cardsetion(player2);
      }
      if (eventInfo?.length) {
        game.broadcastAll(
  ...
  $giveAuto(card, player2) {
    if (Array.isArray(card) && card.length == 0) {
      return;
    }
    var args = Array.from(arguments);
    if (_status.connectMode || !this.isUnderControl(true) && !player2.isUnderControl(true)) {
  ...
  $give(card, player2, log, init, cardsetion) {
    if (!cardsetion && cardsetion !== false && lib.config.card_animation_info) {
      let evt = get.cardsetion(null, true);
      if (evt && evt.player == player2) {
        cardsetion = get.cardsetion(player2);
      } else {
        cardsetion = get.cardsetion(this);
      }
    }
    if (init !== false) {
      game.broadcast(
        function(source, card2, player3, init2, cardsetion2) {
          source.$give(card2, player3, false, init2, cardsetion2);
        },
        this,
        card,
        player2,
      ...
      if (typeof card == "number" && card >= 0) {
        game.addVideo("give", this, [card, player2.dataset.position]);
      } else {
        if (get.itemtype(card) == "card") {
          card = [card];
        }
        if (get.itemtype(card) == "cards") {
          game.addVideo("giveCard", this, [get.cardsInfo(card), player2.dataset.position]);
        }
      }
    }
    if (get.itemtype(card) == "cards") {
      if (log != false && !_status.video) {
        game.log(player2, "从", this, "获得了", card);
      }
      if (this.$givemod) {
        this.$givemod(card, player2);
      } else {
        for (var i = 0; i < card.length; i++) {
          this.$give(card[i], player2, false, false, cardsetion);
        }
      }
    } else if (typeof card == "number" && card >= 0) {
      if (log != false && !_status.video) {
        game.log(player2, "从", this, "获得了" + get.cnNumber(card) + "张牌");
      }
      if (this.$givemod) {
        this.$givemod(card, player2);
      } else {
        while (card--) {
          this.$give("", player2, false, false, cardsetion);
        }
      }
    } else {
      if (log != false && !_status.video) {
        if (get.itemtype(card) == "card" && log != false) {
          ...
          game.log(player2, "从", this, "获得了一张牌");
        }
      }
      if (this.$givemod) {
        this.$givemod(card, player2);
        ...
        node.listenTransition(function() {
          var dx = player2.getLeft() + player2.offsetWidth / 2 - 52 - node.offsetLeft;
          var dy = player2.getTop() + player2.offsetHeight / 2 - 52 - node.offsetTop;
          if (node.style.transform && node.style.transform != "none" && node.style.transform.indexOf("translate") == -1) {
            node.style.transform += " translate(" + dx + "px," + dy + "px)";
          } else {
            node.style.transform = "translate(" + dx + "px," + dy + "px)";
  ...
  addVirtualJudge(card, cards) {
    ...
    const player2 = this;
    game.broadcast(
      (player3, card2, cards2) => {
        player3.addVirtualJudge(card2, cards2);
      },
      player2,
      card,
      cards
    );
    game.addVideo("addVirtualJudge", player2, [get.vcardInfo(cardx), get.cardsInfo(cards)]);
    player2.vcardsMap?.judges.push(cardx);
    if (_status.discarded) {
      _status.discarded.removeArray(cards);
    }
    player2.$addVirtualJudge(card, cards);
  ...
  $addVirtualJudge(VCard, cards) {
    ...
    game.broadcastAll(
      (player3, cardx2, isViewAsCard2, VCard2, cards2) => {
        cardx2.fix();
        ...
        if (isViewAsCard2 && !cardx2.isViewAsCard) {
          ...
          for (let i of cards2) {
            i.goto(ui.special);
            i.destiny = player3.node.judges;
          }
          if (cardx2.destroyed) {
            cardx2._destroyed_Virtua = cardx2.destroyed;
          }
          cardx2.destroyed = function(card2, id, player4, event) {
            if (card2._destroyed_Virtua) {
              if (typeof card2._destroyed_Virtua == "function") {
                let bool = card2._destroyed_Virtua(card2, id, player4, event);
                if (bool === true) {
                  return true;
                }
              } else if (lib.skill[card2._destroyed_Virtua]) {
                if (player4) {
                  if (player4.hasSkill(card2._destroyed_Virtua)) {
        ...
        player3.node.judges.insertBefore(cardx2, player3.node.judges.firstChild);
        ui.updatej(player3);
      },
      player2,
      cardx,
  ...
  addVirtualEquip(card, cards) {
    ...
    game.broadcast(
      (player3, card2, cards2) => {
        player3.addVirtualEquip(card2, cards2);
      },
      player2,
      card,
      cards
    );
    game.addVideo("addVirtualEquip", player2, [get.vcardInfo(cardx), get.cardsInfo(cards)]);
    player2.vcardsMap?.equips.push(cardx);
    player2.vcardsMap?.equips.sort((a, b) => {
      return get.equipNum(a) - get.equipNum(b);
    });
    player2.$addVirtualEquip(card, cards);
    player2.addEquipTrigger(card);
  }
  $addVirtualEquip(card, cards) {
    if (game.online) {
      return;
    ...
    game.broadcastAll(
      (player3, cardx2, isViewAsCard2, card2, cards2, cardShownName2) => {
        cardx2.fix();
        if (!cardx2.isViewAsCard) {
          const cardSymbol = Symbol("card");
          cardx2.cardSymbol = cardSymbol;
        ...
        if (isViewAsCard2 && !cardx2.isViewAsCard) {
          ...
          for (let i of cards2) {
            i.goto(ui.special);
            i.destiny = player3.node.equips;
          }
          if (cardx2.destroyed) {
            cardx2._destroyed_Virtua = cardx2.destroyed;
          }
          cardx2.destroyed = function(card3, id, player4, event) {
            if (card3._destroyed_Virtua) {
              if (typeof card3._destroyed_Virtua == "function") {
                let bool = card3._destroyed_Virtua(card3, id, player4, event);
                if (bool === true) {
                  return true;
                }
              } else if (lib.skill[card3._destroyed_Virtua]) {
                if (player4) {
                  if (player4.hasSkill(card3._destroyed_Virtua)) {
        ...
        if (player3.node.equips.childNodes.length) {
          for (let i = 0; i < player3.node.equips.childNodes.length; i++) {
            if (get.equipNum(player3.node.equips.childNodes[i]) >= equipNum) {
              equipped = true;
              player3.node.equips.insertBefore(cardx2, player3.node.equips.childNodes[i]);
              break;
            }
          }
        }
        if (equipped === false) {
          player3.node.equips.appendChild(cardx2);
  ...
  $equip(card) {
    ...
    var equipped = false;
    for (var i = 0; i < player2.node.equips.childNodes.length; i++) {
      if (get.equipNum(player2.node.equips.childNodes[i]) >= equipNum) {
        player2.node.equips.insertBefore(card, player2.node.equips.childNodes[i]);
        equipped = true;
        break;
      }
    }
    ...
    if (info.skills) {
      for (var i = 0; i < info.skills.length; i++) {
        player2.addSkillTrigger(info.skills[i]);
      }
    }
    return player2;
  }
  $gain(card, log, init, cardsetion) {
    ...
    if (init !== false) {
      game.broadcast(
        function(player3, card2, init2) {
          player3.$gain(card2, false, init2, cardsetion);
        },
        this,
        card,
        init,
  ...
  $gain2(cards, log, cardsetion) {
    ...
    game.broadcast(
      function(player2, cards2, cardsetion2) {
        player2.$gain2(cards2, null, cardsetion2);
      },
      this,
      cards,
      cardsetion
  ...
  $fullscreenpop(str, nature, avatar, broadcast) {
    if (broadcast !== false) {
      game.broadcast(
        function(player2, str2, nature2, avatar2) {
          player2.$fullscreenpop(str2, nature2, avatar2);
        },
        this,
        str,
        nature,
  ...
  $damagepop(num, nature = "soil", font, nobroadcast) {
    if (typeof num == "number" || typeof num == "string") {
      ...
      if (nobroadcast !== false) {
        game.broadcast(
          function(player2, num2, nature2, font2) {
            player2.$damagepop(num2, nature2, font2);
          },
          this,
          num,
          nature,
  ...
  $damage(source) {
    ...
    game.broadcast(
      function(player2, source2) {
        player2.$damage(source2);
      },
      this,
      source
    );
  ...
  $phaseJudge(card) {
    ...
    if (card[card.cardSymbol]?.cards?.length) {
      ...
      const clone = player2.$throw(cards);
      ...
      const clone = player2.$throw(VCard);
