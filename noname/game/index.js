const __vite__mapDeps=(i,m=__vite__mapDeps,d=(m.f||(m.f=["_virtual/typescript.js","_virtual/_commonjsHelpers.js","node_modules/.pnpm/typescript@5.9.3/node_modules/typescript/lib/typescript.js","_virtual/_commonjs-dynamic-modules.js","_virtual/typescript2.js","_virtual/___vite-browser-external.js","_virtual/__vite-browser-external.js"])))=>i.map(i=>d[i]);
import { __vitePreload } from "../../_virtual/preload-helper.js";
import "../../noname.js";
import { isClass, nonameInitialized, userAgentLowerCase, delay } from "../util/index.js";
...
class Game extends GameCompatible {
  documentZoom;
  online = false;
  onlineID = null;
  onlineKey = null;
  ...
  washCard() {
    ...
    cards2.randomSort();
    return game.cardsGotoPile(cards2, "triggeronly", "washCard", ["shuffleNumber", game.shuffleNumber]);
  }
  /**
   * 基于钩子的添加势力方法
   */
  addGroup(id, short, name2, config, type = "default") {
    ...
    if (lib.comparator.typeEquals(name2, "object")) {
      config = name2;
      name2 = null;
    }
   ...
   */
  addCardKnower(cards2, players) {
    if (get.itemtype(cards2) == "card") {
      cards2 = [cards2];
    }
    cards2.forEach((card) => card.addKnower(players));
  }
  /**
   * 移除牌的所有知情者。
   * @param { Card[] | Card } cards
   */
  clearCardKnowers(cards2) {
    if (get.itemtype(cards2) == "card") {
      cards2 = [cards2];
    }
    cards2.forEach((card) => card.clearKnowers());
  }
  /**
   ...
   * @returns { GameHistory[T] }
   */
  getRoundHistory(key, filter = lib.filter.all, num2 = 0, keep, last) {
  ...
  cardsDiscard(cards2) {
    ...
    next.cards = cards2.slice(0).map((i) => i.cards ? i.cards : [i]).flat();
    ...
    next.getd = function(player, key, position) {
      ...
      game.checkGlobalHistory("cardMove", function(evt) {
        ...
        if (position == ui.ordering && evt.relatedEvent == event2.getParent(2) || event2.getParent(2).childEvents.find((evtx) => {
          ...
          cards3.addArray(evt[key]);
        }
      });
      return cards3.filter((c) => event2.cards.includes(c));
    };
    return next;
  }
  /**
   ...
   * @returns { GameEvent }
   */
  cardsGotoOrdering(cards2) {
    var type = get.itemtype(cards2);
    if (type != "cards" && type != "card") {
      return;
    }
    var next = game.createEvent("cardsGotoOrdering");
    next.cards = type == "cards" ? cards2.slice(0) : [cards2];
  ...
  $cardsGotoPile(event2) {
    ...
    for (let i = 0; i < cards2.length; i++) {
      if (cards2[i].willBeDestroyed("cardPile", null, event2)) {
        cards2[i].selfDestroy(event2);
        continue;
      }
      if (event2.insert_index) {
        cards2[i].fix();
        pile.insertBefore(cards2[i], event2.insert_index(event2, cards2[i]));
      } else if (event2.insert_card) {
        cards2[i].fix();
        pile.insertBefore(cards2[i], pile.firstChild);
      } else {
        cards2[i].fix();
        pile.appendChild(cards2[i]);
      }
    }
  ...
  countChoose(clear) {
    ...
    if (_status.connectMode && !_status.countDown) {
      ...
      if (_status.event?.name == "chooseToUse" && _status.event.type == "phase" && typeof info?.phaseUse == "number") {
        ...
        num2 = info[_status.event.name];
      } else if (typeof info?.default == "number") {
        num2 = info.default;
      } else if (_status.connectMode) {
        num2 = lib.configOL.choose_timeout;
      } else {
        num2 = get.config("choose_timeout");
      }
      game.countDown(parseInt(num2), () => {
        ui.click.auto();
        ui.timer.hide();
      });
  ...
  connect(ip, callback) {
    ...
    game.sandbox = _exports.createSandbox(ip);
  ...
  async loadExtension(object) {
    ...
    const name2 = object.name, extensionName = `extension_${name2}`, extensionMenu = lib.extensionMenu[extensionName] = {
      enable: {
        name: "开启",
        init: true
      }
    ...
    const config = Object.keys(libConfig).reduce((constructingConfig, key) => {
      if (key != extensionName && key.startsWith(extensionName)) {
        constructingConfig[key.slice(11 + name2.length)] = libConfig[key];
      }
      return constructingConfig;
    }, {});
    try {
      let extensionPack = lib.extensionPack[name2];
      if (objectPackage) {
        extensionPack = lib.extensionPack[name2] = objectPackage;
        ...
        extensionPack = lib.extensionPack[name2] = {};
      }
      const arenaReady = object.arenaReady, content = object.content, prepare = object.prepare, precontent = object.precontent;
      extensionPack.code = {
        arenaReady,
        content,
        prepare,
      ...
      try {
        ...
        console.error(`加载《${name2}》扩展的precontent时出现错误。`, e1);
...
${decodeURI(e1.stack)}`);
      ...
      if (content) {
        lib.extensions.push([name2, content, config, _status.evaluatingExtension, objectPackage ?? {}, object.connect, arenaReady]);
      }
    } catch (e) {
      console.error(e);
    }
    ...
    if (get.objtype(data) == "object") {
      ...
      if (extensionPackage) {
        ...
        if (size < 1e3) {
          ...
          extensionPackage.size = `${Math.round(size / 1e3)}KB`;
        } else {
          extensionPackage.size = `${Math.round(size / 1e5) / 10}MB`;
        }
        zip.file(
          "package.js",
...
`)
      ...
      if (lib.device) {
        ...
        new Promise((resolve, reject) => window.resolveLocalFileSystemURL(directory, resolve, reject)).then(
          (directoryEntry) => new Promise(
            (resolve, reject) => directoryEntry.getFile(
              fileNameToSaveAs,
              {
                create: true
              },
        ...
        ).then((fileEntry) => new Promise((resolve, reject) => fileEntry.createWriter(resolve, reject))).then(
...
${JSON.stringify(error, null, "	")}`);
    ...
    try {
      ...
      if (isTsFile) {
        if (typeof globalThis.ts === "undefined") {
          globalThis.ts = await __vitePreload(() => import("../../_virtual/typescript.js").then((n) => n.t), true ? __vite__mapDeps([0,1,2,3,4,5,6]) : void 0);
        }
      ...
      try {
        ...
        if (
          // @ts-expect-error ignore
          error.message === "Cannot use import statement outside a module" || // @ts-expect-error ignore
      ...
      if (typeof game.readFile == "function") {
        ...
        if (lib.node && lib.node.fs) {
          const writeFile = (errnoException) => {
            ...
            if (fileList.length) {
              const zipDir = fileList.pop(), fileName = zipDir.split("/"), name2 = fileName.pop(), letGo = (name3) => new Promise((resolve) => lib.node.fs.writeFile(`${__dirname}/extension/${extensionName}/${name3}`, zip.file(zipDir).asNodeBuffer(), null, resolve)).then(writeFile);
              return fileName.length ? game.promises.createDir(`extension/${extensionName}/${fileName.join("/")}`).then(() => letGo(`${fileName.join("/")}/${name2}`)) : letGo(name2);
            }
            finishLoad();
          };
          ...
          new Promise((resolve, reject) => window.resolveLocalFileSystemURL(nonameInitialized, resolve, reject)).then(
            (directoryEntry) => new Promise(
              (resolve, reject) => directoryEntry.getDirectory(
            ...
            const writeFile = () => {
              ...
              const zipDirectory = fileList.shift(), fileName = zipDirectory.split("/"), name2 = fileName.pop(), letGo = (name3) => new Promise(
                (resolve, reject) => directoryEntry.getFile(
                  name3,
                  {
                    create: true
          ...
          const writeFile = (errnoException) => {
            ...
            if (fileList.length) {
              const zipDir = fileList.pop(), fileName = zipDir.split("/"), name2 = fileName.pop(), letGo = (name3) => game.promises.writeFile(zip.file(zipDir).asArrayBuffer(), `extension/${extensionName}`, name3).then(writeFile);
              return fileName.length ? game.promises.createDir(`extension/${extensionName}/${fileName.join("/")}`).then(() => letGo(`${fileName.join("/")}/${name2}`)) : letGo(name2);
            }
            finishLoad();
          };
   ...
   * @param { string } [name]
   */
  export(textToWrite, name2) {
    let textFileAsBlob = new Blob([textToWrite], { type: "text/plain" });
    let fileNameToSaveAs = name2 || "noname";
  ...
  multiDownload(list, onsuccess, onerror, onfinish, process, dev) {
    ...
    if (list.length <= 3) {
      ...
      let num2 = Math.round(list.length / 3);
      let left2 = 3;
      args[3] = function() {
        left2--;
        if (left2 == 0) {
      ...
      setTimeout(function() {
        args[0] = list.slice(0, num2);
        game.multiDownload2.apply(game, args);
      });
      setTimeout(function() {
        args[0] = list.slice(num2, 2 * num2);
        game.multiDownload2.apply(this, args);
      }, 200);
      setTimeout(function() {
        args[0] = list.slice(2 * num2);
        game.multiDownload2.apply(this, args);
      }, 400);
    }
  }
  ...
  videoContent = {
    ...
    addVirtualEquip: function(player, map) {
      const card = get.infoVCard(map[0]), cards2 = get.infoCards(map[1]);
      player.addVirtualEquip(card, cards2);
    },
    addVirtualJudge: function(player, map) {
      const card = get.infoVCard(map[0]), cards2 = get.infoCards(map[1]);
      player.addVirtualJudge(card, cards2);
    },
    ...
    chessgainmod: function(player, num2) {
      if (Array.isArray(num2)) {
        num2 = get.infoCards(num2);
      }
    ...
    showCharacter: function(player, num2) {
    ...
    hideCharacter: function(player, num2) {
      if (player && player.classList) {
        switch (num2) {
          case 0:
      ...
      if (player && player.$draw) {
        ...
        for (var i = 0; i < cards2.length; i++) {
          for (var j = 0; j < nodes.length; j++) {
            if (cards2[i][2] == nodes[j].name && cards2[i][0] == nodes[j].suit && cards2[i][1] == nodes[j].number) {
        ...
        if (cards2.length) {
          player.$draw(get.infoCards(cards2));
        }
      } else {
        console.log(player);
      }
    },
    deletenode: function(player, cards2, method) {
      if (cards2) {
        var nodeList = document.querySelectorAll("#arena>.card,#chess>.card");
        var nodes = [];
        for (var i = 0; i < nodeList.length; i++) {
    ...
    diex: function(player) {
      ...
      var cards2 = player.getCards("hej");
      for (var i = 0; i < cards2.length; i++) {
        cards2[i].discard();
      }
    ...
    directgain: function(player, cards2) {
      if (player && cards2) {
        player.directgain(get.infoCards(cards2));
      } else {
        console.log(player);
      }
    },
    directgains: function(player, cards2) {
      if (player && cards2) {
        if (get.is.object(cards2)) {
          player.directgains(get.infoCards(cards2.cards), null, cards2.gaintag);
        } else {
          player.directgains(get.infoCards(cards2));
        }
      } else {
        console.log(player);
      }
    },
    directequip: function(player, cards2) {
      if (player && cards2) {
        player.directequip(get.infoCards(cards2));
      } else {
        console.log(player);
      }
    },
    ...
    swapSeat: function(content) {
      ...
      if (player1.dataset.position == "0" || player2.dataset.position == "0") {
        ...
        num2 = game.players.length + game.dead.length;
    ...
    swapPlayer: function(player, hs) {
      if (player && player.node) {
        ...
        var num2 = game.players.length + game.dead.length;
        var players = game.players.concat(game.dead);
        var temp;
        for (var i = 0; i < players.length; i++) {
  ...
  addRecentCharacter() {
    ...
    let num2 = parseInt(lib.config.recent_character_number);
    if (list.length > num2) {
      list.splice(num2);
    }
  ...
  check(event2 = _status.event) {
    ...
    let useCache = !lib.config.compatiblemode && !event2.skill && !event2.multitarget;
  ...
  uncheck(...args) {
    ...
    if (args.includes("card") && event2.player) {
      const cards2 = event2.player.getCards("hejsx");
      cards2.forEach((card) => {
  ...
  swapPlayer(player, player2) {
    ...
    if (player2) {
      ...
      if (!game.chess) {
        ...
        let num2 = game.players.length + game.dead.length;
  ...
  chooseCharacterDouble() {
    ...
    let config, width, num2, ratio, func, update, list, first;
    for (let i = 0; i < arguments.length; i++) {
   ...
   */
  asyncDraw(players, num2, drawDeck, bottom) {
    return Promise.all(
      players.map((player, index) => {
        ...
        if (drawDeck && drawDeck.drawDeck) {
          return player.draw(num22, false, drawDeck);
        }
        if (bottom) {
          return player.draw(num22, "nodelay", "bottom");
        }
        return player.draw(num22, "nodelay");
      })
    );
  }
  /**
   ...
   * @param { { drawDeck: boolean } } [drawDeck]
   */
  asyncDrawAuto(players, num2, drawDeck) {
    if (players.length > 1) {
    ...
    if (typeof num2 == "number") {
      ...
      num22 = num2(players[0]);
    }
    if (drawDeck && drawDeck.drawDeck) {
      players[0].draw(num22, drawDeck);
    } else {
      players[0].draw(num22);
    }
  }
  finishSkill(i, sub) {
      ...
      lib.skill[k] = /* @__PURE__ */ ((round, name2) => ({
        ...
        intro: {
          content: (storage, player) => {
            ...
            const num2 = round - (game.roundNumber - storage);
            if (num2 > 0) {
              str += `${get.cnNumber(num2)}轮后${info2.roundtext || "技能重置"}`;
            } else {
              str += "技能可发动";
            }
            return str;
  ...
  logv(player, card, targets, event2, forced, logvid) {
    ...
    const fullheight = ui.historybar.offsetHeight, num2 = Math.round((fullheight - 8) / 50), margin = (fullheight - 42 * num2) / (num2 + 1);
    ...
    if (lib.version != lib.config.version) {
      lib.changeLog.forEach((value) => {
        if (value.startsWith("players://")) {
          ...
          try {
            cards2 = JSON.parse(value.slice(8)).filter((value2) => lib.card[value2]);
          } catch (e) {
            cards2 = null;
          }
        } else {
   ...
   * @returns { Card[] }
   */
  createFakeCards(cards2, isBlank = false, tempname) {
   ...
   */
  deleteFakeCards(cards2) {
    if (!Array.isArray(cards2)) {
      cards2 = [cards2];
    }
    const fake = cards2.filter((card) => card.isFake && card.rcardSymbol && card._cardid), other = cards2.removeArray(fake), wild = [], map = {};
    fake.forEach((card) => {
