...
import { pinyin, convert, addDict } from "../../node_modules/.pnpm/pinyin-pro@3.27.0/node_modules/pinyin-pro/dist/index.js";
import dictionary from "./pinyins/noname-dict.js";
import { Audio } from "./audio.js";
import { AsyncGeneratorFunction, AsyncFunction, GeneratorFunction } from "../util/index.js";
import _exports from "../util/security.js";
import { ErrorManager, CodeSnippet } from "../util/error.js";
import JSZip from "../../_virtual/index4.js";
...
class Get extends GetCompatible {
  is = new Is();
  ...
  addNewRowList(cards, type = "suit", player = false) {
    function createCustom() {
      return function(itemContainer) {
        ...
        const innerHTML = count ? formatStr(`${get.translation(link)}牌${count}张`, type2) : formatStr(`没有${get.translation(link)}牌`, type2);
        div.css({
          innerHTML,
          position: "absolute",
          width: "100%",
  ...
  cardsetion(player, sourceEvent) {
    ...
    let name1 = name, name2 = _status.event.getParent().name, th_skill = false, evt1 = get.event(), evt2 = get.event().getParent();
    ...
    const banned = ["phaseJudge", "equip", "phaseUse"], bannedEvent = ["phase"];
    ...
    switch (name) {
      ...
      case "lose": {
        let event = _status.event, evt = event.getParent();
    ...
    for (const name of lib.inpile) {
      ...
      if (name == "sha") {
        for (const nature of lib.inpile_nature) {
          const info2 = [type, "", name, nature];
          if (!filter || filter(info2)) {
            list.push(info2);
          }
        }
      }
  ...
  pinyin(chinese, withTone) {
    ...
    if (pinyins && pinyins[chinese] && Array.isArray(pinyins[chinese])) {
      ...
      result = pinyin(chinese, { type: "array" });
    }
    if (withTone === false) {
      result = convert(result, { format: "toneNone" });
    }
    return result;
  }
  /**
   ...
   */
  yunmu(str) {
    if (lib.pinyins._metadata.zhengtirendu.includes(convert(str, { format: "toneNone" }))) {
   ...
   */
  yunjiao(str) {
    str = convert(str, { format: "toneNone" });
  ...
  bingzhu(name) {
    ...
    const list = [], info = lib.card[name];
    if (lib.cardBingzhu[name]) {
      list.addArray(lib.cardBingzhu[name]);
    }
    if (info.derivation) {
      const names = get.characterSurname(info.derivation).map((list2) => list2.join(""));
      list.addArray(names);
    }
    if (info.bingzhu) {
  ...
  character(name, num) {
    ...
    if (!info) {
      const pack = Object.keys(lib.characterPack).find((pack2) => name in lib.characterPack[pack2]);
      if (pack) {
   ...
   */
  copy(obj, copyKeyDeep = false, map = /* @__PURE__ */ new WeakMap()) {
    const getType = (obj2) => Object.prototype.toString.call(obj2);
    const canTranverse = {
      "[object Map]": true,
      "[object Set]": true,
      "[object Object]": true,
    ...
    const target = constructor ? Array.isArray(obj) || obj instanceof Map || obj instanceof Set || constructor === Object ? (
  ...
  id() {
    return Math.floor(1e6 + 9e6 * Math.random()).toString() + (10 + lib.status.globalId++);
  }
  ...
  rank(name, num) {
    ...
    const RANK_MAP = [
      ...
      { key: "bp", factor: 4, ref: lib.rank.bp },
      // 中位数等级
  ...
  skillRank(skill, type, grouped) {
    ...
    if (info.ai) {
      ...
      if (player?.hasSkill && info?.ai?.combo) {
        ...
        if (!skills.every((skill2) => player.hasSkill(skill2, null, null, false))) {
          return 0;
        }
      }
      if (info.ai.neg) {
  ...
  infoVCard(card) {
    ...
    if (vid in lib.vcardOL) {
      ...
      const card2 = new lib.element.VCard(datas);
      lib.vcardOL[vid] = card2;
      return card2;
    }
  }
  infoVCards(infos) {
  ...
  isFunctionParam(paramstr) {
    ...
    const canCreateFunction = _exports.isSandboxRequired() && _exports.importSandbox().Marshal.canCreateFunction;
  ...
  isFunctionBody(code, type = (
    ...
    if (type == "any") {
      return ["async", "generator", "agenerator", null].some((t) => get.isFunctionBody(code, t));
    }
    try {
      switch (type) {
        default:
  ...
  funcInfoOL(func) {
    if (typeof func == "function") {
      ...
      const decompileFunction = _exports.isSandboxRequired() ? _exports.importSandbox().Marshal.decompileFunction : Function.prototype.call.bind(Function.prototype.toString);
  ...
  infoFuncOL(info) {
    ...
    const str = get.pureFunctionStr(info.slice(13), true);
    if ("sandbox" in window) {
    ...
    try {
      ...
      if (_exports.isSandboxRequired()) {
        ...
        const box = _exports.currentSandbox();
        if (!box) {
...
${str}`);
  ...
  eventInfoOL(item, level, noMore) {
    return get.itemtype(item) == "event" ? `_noname_event:${JSON.stringify(
  ...
  vcardInfoOL(item) {
    return "_noname_vcard:" + JSON.stringify(
      Object.entries(item).reduce((stringifying, entry) => {
        const key = entry[0];
  ...
  prefixSpan(prefix, name) {
    ...
    if (config == "simple") {
      ...
      span2.innerHTML = prefix;
      return span2.outerHTML;
    }
  ...
  time() {
    if (lib.status.dateDelaying) {
      ...
      return lib.getUTC(/* @__PURE__ */ new Date()) - lib.getUTC(lib.status.date) - lib.status.dateDelayed;
    }
  }
  utc() {
  ...
  type(obj, method, player) {
    ...
    if (!lib.card[name]) {
      ...
      if (name.slice(4).split("_").every((n) => lib.nature.has(n))) {
  ...
  card(original) {
    if (_status.event.skill) {
      ...
      let card2 = info.viewAs;
      if (typeof card2 == "function") {
        card2 = card2(ui.selected.cards, _status.event.player);
      }
      if (card2) {
        if (!ui.selected.cards.length) {
          return get.autoViewAs(card2, card2.cards);
        }
        return get.autoViewAs(card2, ui.selected.cards);
      }
    }
  ...
  nameList(player) {
    ...
    return ["name", "name1", "name2"].filter((prop) => player[prop]).map((prop) => player[prop]).toUniqued();
  }
  ...
  skillTranslation(str, player, prefix = false) {
    ...
    if (info?.duplicatePrefix !== void 0) {
      const prefix2 = info.duplicatePrefix;
      if (typeof prefix2 == "function") {
        return `${prefix2(player, str)}${name}`;
      }
      return `${prefix2}${name}`;
    }
    ...
    if (_status.skillOwner?.[str] !== void 0) {
  ...
  skillInfoTranslation(name, player, noHTML = true) {
    let str = (() => {
      ...
      const str2 = lib.translate[name + "_info"];
      if (!str2) {
        return "";
      }
      return str2;
    ...
    if (str && typeof str == "object" && (str.name || str._tempTranslate)) {
      ...
      if (str2 == "杀") {
        ...
        if (typeof str.nature == "string") {
          ...
          for (let nature2 of natures) {
            str2 += lib.translate["nature_" + nature2] || lib.translate[nature2] || "";
          }
        }
        str2 += "杀";
      }
  ...
  menuZoom() {
    if (game.menuZoom === void 0 || game.menuZoom === null) {
  ...
  cnNumber(num, ordinal) {
    ...
    const handleZero = (str) => {
      let result2 = str.replace(/零{2,}/g, "零");
      if (result2.length > 1) {
        result2 = result2.replace(/零+$/g, "");
      }
      return result2;
    };
    const _transform = (str) => {
      ...
      for (let i = 0; i < str.length; i++) {
        ...
        result2 = char + unit + result2;
      }
      result2 = handleZero(result2);
      return result2;
    };
    let result = "";
    let tempYi = "";
  ...
  *iterableChildNodes(node) {
  ...
  population(identity) {
    return identity == void 0 ? game.players.length + game.dead.length : game.players.filter((current) => current.identity == identity).length;
  }
  totalPopulation(identity) {
    return identity == void 0 ? game.players.length + game.dead.length : game.players.concat(game.dead).filter((current) => current.identity == identity).length;
  }
  /**
    ...
    if (typeof node._customintro == "function") {
      ...
      if (!simple || get.is.phoneLayout()) {
        ...
        viewInfo.listen(function() {
          ...
          ui.click.charactercard(player2.name1 || player2.name, null, null, true, player2.node, audioName);
        });
      ...
      if (!simple || get.is.phoneLayout()) {
        if ((lib.config.change_skin || lib.skin) && !node.isUnseen()) {
          ...
          var createButtons = function(num2, avatar2) {
            if (!introadded) {
              introadded = true;
            ...
            var nameskin3 = avatar2 ? node.name2 : node.name1;
            var nameskin22 = nameskin3;
            var gzbool2 = false;
            if (nameskin3.startsWith("gz_shibing")) {
              nameskin3 = nameskin3.slice(3, 11);
            } else if (nameskin3.startsWith("gz_")) {
              nameskin3 = nameskin3.slice(3);
              gzbool2 = true;
            }
            for (var i2 = 0; i2 <= num2; i2++) {
              var button = ui.create.div(".button.character.pointerdiv", buttons, function() {
                if (this._link) {
                  if (avatar2) {
                    lib.config.skin[nameskin3] = this._link;
                  ...
                  delete lib.config.skin[nameskin3];
                  if (avatar2) {
                    if (gzbool2 && lib.character[nameskin22].hasSkinInGuozhan && lib.config.mode_config.guozhan.guozhanSkin) {
                      node.node.avatar2.setBackground(nameskin22, "character");
                    } else {
                      node.node.avatar2.setBackground(nameskin3, "character");
                    }
                  } else {
                    if (gzbool2 && lib.character[nameskin22].hasSkinInGuozhan && lib.config.mode_config.guozhan.guozhanSkin) {
                      node.node.avatar.setBackground(nameskin22, "character");
                    } else {
                      node.node.avatar.setBackground(nameskin3, "character");
                    }
                  }
                }
              ...
              button._link = i2;
              if (i2) {
                button.setBackgroundImage("image/skin/" + nameskin3 + "/" + i2 + ".jpg");
              } else {
                if (gzbool2 && lib.character[nameskin22].hasSkinInGuozhan && lib.config.mode_config.guozhan.guozhanSkin) {
                  button.setBackground(nameskin22, "character", "noskin");
                } else {
                  button.setBackground(nameskin3, "character", "noskin");
                }
              }
            }
            uiintro.add(buttons);
          ...
          var loadImage = function(avatar2) {
            ...
            img.src = lib.assetURL + "image/skin/" + nameskin3 + "/" + num + ".jpg";
          };
          if (lib.config.change_skin) {
            ...
            setTimeout(function() {
              ...
              var nameskin22 = node.name2;
              if (nameskin1 && nameskin1.startsWith("gz_")) {
                nameskin1 = nameskin1.slice(3);
              }
              if (nameskin22 && nameskin22.startsWith("gz_")) {
                nameskin22 = nameskin22.slice(3);
              }
              if (!node.isUnseen(0) && lib.skin[nameskin1]) {
                createButtons(lib.skin[nameskin1]);
              }
              if (!node.isUnseen(1) && lib.skin[nameskin22]) {
                createButtons(lib.skin[nameskin22], true);
              }
            });
          }
        }
      ...
      if (node._banning) {
        ...
        if (addskin && (!simple || get.is.phoneLayout())) {
          ...
          var createButtons = function(num2) {
            if (!num2) {
              return;
            }
            if (!introadded) {
            ...
            for (var i2 = 0; i2 <= num2; i2++) {
              ...
              button._link = i2;
              if (i2) {
                button.setBackgroundImage("image/skin/" + nameskin + "/" + i2 + ".jpg");
              } else {
                if (gzbool && lib.character[nameskin2].hasSkinInGuozhan && lib.config.mode_config.guozhan.guozhanSkin) {
  ...
  poptipIntro(info, poptip, event) {
    ...
    const adjust = function() {
      ...
      if (uiintro._poptipOriginalTransform === void 0) {
    ...
    const removeObserver = new MutationObserver((mutations) => {
      for (const m2 of mutations) {
        for (const node of m2.removedNodes) {
          if (node === uiintro) {
  ...
  useful_raw(card, player) {
    ...
    if (player) {
      if (_status.event.useCache) {
        i = game.callFuncUseStepCache(
          "player.getCardsInUseful",
          function(player2, position, cardname) {
            return player2.getCards(position, cardname);
          },
          [player, "h", card.name]
        ).indexOf(card);
      } else {
  ...
  value(card, player, method) {
    ...
    if (player == void 0 || get.itemtype(player) != "player") {
      player = _status.event.player;
    }
    var geti = function() {
  ...
  equipValue(card, player) {
    ...
    if (value == void 0) {
      if (info.ai.basic && info.ai.basic.equipValue != void 0) {
  ...
  effect_use(target, card, player, player2, isLink) {
    ...
    if (typeof card != "string" && (typeof card != "object" || !card.name)) {
      ...
      if (event.skill && skillinfo.viewAs == void 0) {
    ...
    for (let i2 = 0; i2 < skills1.length; i2++) {
      const info2 = get.info(skills1[i2]);
      if (!info2) {
        throw new Error(`${skills1[i2]}不存在的技能`);
      }
      temp1 = info2.ai;
    ...
    if (target) {
      ...
      for (let i2 = 0; i2 < skills2.length; i2++) {
        const info2 = get.info(skills2[i2]);
        if (!info2) {
          throw new Error(`${skills2[i2]}不存在的技能`);
        }
        temp2 = info2.ai;
  ...
  effect(target, card, player, player2, isLink) {
    ...
    for (var i = 0; i < skills1.length; i++) {
      const info2 = get.info(skills1[i]);
      if (!info2) {
    ...
    if (target) {
      ...
      for (var i = 0; i < skills2.length; i++) {
        const info2 = get.info(skills2[i]);
        if (!info2) {
        ...
        if (typeof temp3 == "function" && temp3(player, target) != void 0) {
          threaten *= temp3(player, target);
        } else if (typeof temp3 == "object") {
          if (typeof temp3.target == "number") {
            threaten *= temp3;
          } else if (typeof temp3.target == "function" && temp3(player, target) != void 0) {
            threaten *= temp3(player, target);
          }
